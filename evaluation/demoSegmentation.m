...
addpath '/home/tony/app/caffe/matlab' 
...
if (strcmp(model_type, 'FCN'))
	model_definition = 'model/deploy_FCN.prototxt';
	model_weights = 'model/FCN_iter_160000.caffemodel';
elseif (strcmp(model_type, 'Dilated')) 
	model_definition = 'model/deploy_DilatedNet.prototxt';
...
pathImg = fullfile('/home/tony/app/sceneparsing/ADEChallengeData2016', 'images','validation');
pathAnno = fullfile('/home/tony/app/sceneparsing/ADEChallengeData2016', 'annotations','validation');
pathPred = fullfile('mc_result', prediction_folder,'gray');
pathrgbPred = fullfile('mc_result', prediction_folder,'color');

if (~exist(pathPred, 'dir'))
	mkdir(pathPred);
end
if (~exist(pathrgbPred,'dir'))
    mkdir(pathrgbPred)
end
...
for i = 1: numel(filesImg)
    ...
    filergbPred = fullfile(pathrgbPred, strrep(filesImg(i).name, '.jpg', '.png'));
