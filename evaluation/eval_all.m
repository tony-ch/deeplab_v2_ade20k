...
switch data_name
    case 'ADE20K'
        ...
        save_root = 'mc_result/ADE20K/plus/iter_40000_f0/val_v1_weighted_strict/'; %root path to store the result image
        %save_root = 'mc_result/predictions_FCN/'; %root path to store the result image
        model_weights = 'model/plus_v1_2_f0/10000/train_iter_12000.caffemodel';
        model_deploy = 'prototxt/test_val.prototxt';
        fea_cha = 150; %number of classes
        base_size = 512; %based size for scaling
        crop_size = 513; %crop size fed into network
...
mean_r = 122.675; %means to be subtracted and the given values are used in our training stage
mean_g = 116.669;
mean_b = 104.008;

acc = double.empty;
iou = double.empty;
gpu_id_array = [0:0]; %multi-GPUs for parfor testing, if number of GPUs is changed, remember to change the variable 'step'
runID = 1;
